...
exports.seekTeam = async (req, res) => {
  ...
  try {
    if (!inviteCode) return res.status(400).json({ success: false, error: { message: "초대 코드를 입력해주세요." } });
    ...
    if (!team) return res.status(404).json({ success: false, error: { message: "해당 코드의 팀을 찾을 수 없습니다." } });
